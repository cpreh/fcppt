...
TEST_CASE("array::map", "[container],[array]")
{
  CHECK(
      fcppt::container::array::map(std::array<int, 2>{{1, 2}}, [](int const _value) -> std::string {
        return fcppt::output_to_std_string(_value);
      }) == std::array<std::string, 2>{{std::string{"1"}, std::string{"2"}}});
}

TEST_CASE("array::map move", "[container],[array]")
{
  ...
  CHECK(
      ...
      std::array<strong_int_movable, 2>{
