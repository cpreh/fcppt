...
using test_list = fcppt::intrusive::list<test_class>;

class test_class : public fcppt::intrusive::base<test_class>
{
  FCPPT_NONCOPYABLE(test_class);

public:
  test_class(
      test_list &_list, // NOLINT(google-runtime-references)
      int const _value)
      : fcppt::intrusive::base<test_class>{_list}, value_{_value}
  {
  }

  test_class(test_class &&) noexcept = default;

  test_class &operator=(test_class &&) noexcept = default;
