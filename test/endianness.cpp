...
TEST_CASE("endianness", "[endianness]")
{
  ...
  CHECK(
      ((fcppt::endianness::is_little_endian() &&
        fcppt::endianness::convert(foo, fcppt::endianness::format::little) == 108) ||
       (fcppt::endianness::is_big_endian() &&
        fcppt::endianness::convert(foo, fcppt::endianness::format::big) == 108)));

  CHECK(fcppt::endianness::swap(fcppt::endianness::swap(foo)) == foo);

  CHECK(
      fcppt::endianness::convert(
          fcppt::endianness::convert(foo, fcppt::endianness::format::big),
          fcppt::endianness::format::big) == 108);

  CHECK(
      fcppt::endianness::convert(
          fcppt::endianness::convert(foo, fcppt::endianness::format::little),
          fcppt::endianness::format::little) == 108);
}
