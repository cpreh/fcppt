...
{
// ![vector2f]
using vector2f = fcppt::math::vector::object<float, 2, std::array<float, 2>>;
// ![vector2f]
}

// ![vector2f_short]
using vector2f = fcppt::math::vector::static_<float, 2>;
// ![vector2f_short]
...
{
  // ![convert]
  fcppt::math::vector::static_<int, 1> const i(10);
...
void push_back_narrow()
{
  // ![push_back_narrow]
  fcppt::math::vector::static_<int, 1> const small(10);
  // Convert smaller to bigger. Only works for N and N+1 and we have to
  // specify the additional element.
  fcppt::math::vector::static_<int, 2> const bigger(fcppt::math::vector::push_back(small, 20));

  // Convert bigger to smaller
  auto const small_again(
      fcppt::math::vector::narrow_cast<fcppt::math::vector::static_<int, 1>>(bigger));
  // ![push_back_narrow]
...
{
  // ![null]
  auto const n(fcppt::math::vector::null<vector2f>());
  // ![null]
  std::cout << n << '\n';
}
...
bool operator==(second, second) { return true; }
...
{
template <>
struct make_literal<second>
{
  using decorated_type = second;

  static decorated_type get(int) { return second{}; }
};
...
{
  // ![asymmetric_div]
  using meter2 = fcppt::math::vector::static_<meter, 2>;

  using second2 = fcppt::math::vector::static_<second, 2>;

  using speed2 = fcppt::math::vector::static_<speed, 2>;
