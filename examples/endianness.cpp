...
{
  //! [endianness_query]
  switch (fcppt::endianness::host_format())
  {
  ...
  // in the host format.
  int const iconverted(fcppt::endianness::convert(ivariable, fcppt::endianness::format::little));
  ...
  std::cout << fcppt::endianness::convert(iconverted, fcppt::endianness::format::little) << '\n';
  //! [endianness_convert]
}
//]
