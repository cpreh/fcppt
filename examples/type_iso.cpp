...
// ![decorate_simple]
	...
	auto const var(
		fcppt::type_iso::decorate<strong_int>(42)
	);

	int const result(
		fcppt::type_iso::undecorate(var)
	);
...
>
{
	...
	decorate(undecorated_type const _val)
	{
		return
			decorated_type::make(_val);
	}

	static undecorated_type
	undecorate(decorated_type const _val)
	{
		return
			_val.get();
	}
