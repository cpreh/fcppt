...
inline fcppt::parse::repetition_plus<std::remove_cvref_t<Parser>> operator+(Parser &&_parser)
  requires(fcppt::parse::is_valid_argument<Parser>::value)
{
  return fcppt::parse::repetition_plus<std::remove_cvref_t<Parser>>{std::forward<Parser>(_parser)};
}
}
