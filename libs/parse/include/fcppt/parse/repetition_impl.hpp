...
fcppt::parse::repetition<Parser>::repetition(Parser &&_parser) : parser_{std::move(_parser)}
{
}

template <typename Parser>
template <typename Ch, typename Skipper>
fcppt::parse::result<Ch, typename fcppt::parse::repetition<Parser>::result_type>
fcppt::parse::repetition<Parser>::parse(
    fcppt::reference<fcppt::parse::basic_stream<Ch>> const _state, Skipper const &_skipper) const
{
  ...
  fcppt::parse::error<Ch> error{fcppt::either::loop(
      ...
      [_state, &result, &pos](fcppt::parse::result_of<Parser> &&_element) {
        pos = fcppt::parse::get_position(_state);

        result.push_back(std::move(_element));
      })};

  fcppt::parse::set_position(_state, pos);

  return error.is_fatal() ? fcppt::either::make_failure<result_type>(std::move(error))
                          : fcppt::parse::make_success<Ch>(std::move(result));
}
