...
namespace detail
{
template <typename Ret, typename Variant>
inline Ret &get_exn_impl(Variant &_variant)
{
  using return_type = std::remove_const_t<Ret>;

  if (!fcppt::variant::holds_type<return_type>(_variant))
  {
    throw fcppt::variant::invalid_get{
        FCPPT_TEXT("Invalid get of type \"") +
        fcppt::from_std_string(fcppt::type_name_from_info(typeid(return_type))) +
        FCPPT_TEXT("\" from variant with types \"") +
        fcppt::variant::types_string<std::remove_const_t<Variant>>() +
        FCPPT_TEXT("\". Current type was \"") +
        fcppt::from_std_string(fcppt::variant::current_type_name(_variant)) + FCPPT_TEXT("\".")};
  }

  return fcppt::variant::get_unsafe<return_type>(_variant);
}
