...
#ifndef FCPPT_MPL_GREATER_HPP_INCLUDED
#define FCPPT_MPL_GREATER_HPP_INCLUDED
...
struct greater<std::integral_constant<T,V1>,std::integral_constant<T,V2>>
{
  using type = std::bool_constant<(V1 > V2)>;
};
}
/**
\brief Checks if one integral constant is greater than another.
\ingroup fcpptmpl
...
using greater = fcppt::mpl::detail::greater<T1,T2>::type;
}
