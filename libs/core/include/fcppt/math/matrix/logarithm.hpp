...
logarithm(fcppt::math::matrix::object<T, N, N, S> A, T const e1, T const e2, T const e3)
{
  ...
  while (fcppt::math::matrix::infinity_norm(A - fcppt::math::matrix::identity<matrix_type>()) > e1)
  {
    A = fcppt::math::matrix::sqrt(A, e3);
    k++;
  }

  A -= fcppt::math::matrix::identity<matrix_type>();

  matrix_type Z{A};

  matrix_type X{A};

  fcppt::math::size_type i{1U};

  while (fcppt::math::matrix::infinity_norm(Z) > e2)
  {
    Z = Z * A;
    ++i;
    X = X + fcppt::literal<T>(1) / fcppt::cast::int_to_float<T>(i) * Z;
  }

  if (k != fcppt::literal<fcppt::math::size_type>(0))
  {
    X = fcppt::cast::int_to_float<T>(fcppt::literal<fcppt::math::size_type>(1U) << k) * X;
  }
