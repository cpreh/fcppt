...
*/
template <typename T>
inline fcppt::optional::object<T> mod(T const &_dividend, T const &_divisor)
{
  return fcppt::optional::make_if(!fcppt::math::is_zero(_divisor), [_dividend, _divisor] {
    return fcppt::math::detail::mod(_dividend, _divisor);
  });
}
