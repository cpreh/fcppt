...
Creates a vector of type \p Vec with all components set to 0 expect for component \p _axis which is
set to 1:
...
{
  return fcppt::math::vector::init<Vec>([_axis](fcppt::math::size_type const _index) {
    return fcppt::literal<fcppt::type_traits::value_type<Vec>>(_index == _axis ? 1 : 0);
  });
}
