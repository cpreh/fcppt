...
Box init(InitSecond const &_init_second, Function const &_function)
{
  ...
  using result_array = std::
      array<fcppt::homogenous_pair<fcppt::type_traits::value_type<Box>>, Box::dim_wrapper::value>;
  ...
  return Box(
      fcppt::math::vector::init<typename Box::vector>([&results](auto const _index) {
        ...
        return std::get<_index()>(results).first;
      }),
      _init_second([&results](auto const _index) {
        FCPPT_USE(_index);

        return std::get<_index()>(results).second;
      }));
}
