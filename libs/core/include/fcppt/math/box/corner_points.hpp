...
std::array<fcppt::math::vector::static_<T, N>, fcppt::math::power_of_2<std::size_t>(N)>
corner_points(fcppt::math::box::object<T, N> const &_box)
{
  using vector_type = fcppt::math::vector::static_<T, N>;

  using result_type = std::array<vector_type, fcppt::math::power_of_2<std::size_t>(N)>;

  result_type const corners(fcppt::math::vector::bit_strings<T, N>());

  return fcppt::container::array::init<result_type>([&_box, &corners](auto const _index) {
    FCPPT_USE(_index);

    return _box.pos() + std::get<_index()>(corners) * fcppt::math::dim::to_vector(_box.size());
  });
}
