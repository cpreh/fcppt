...
template <typename Type>
fcppt::recursive<Type>::recursive(Type &&_value)
    : impl_{fcppt::make_unique_ptr<Type>(std::move(_value))}
{
}

template <typename Type>
fcppt::recursive<Type>::recursive(recursive const &_other)
    : impl_{fcppt::make_unique_ptr<Type>(_other.get())}
{
}

template <typename Type>
fcppt::recursive<Type>::recursive(recursive &&) noexcept = default;

template <typename Type>
fcppt::recursive<Type> &fcppt::recursive<Type>::operator=( // NOLINT(cert-oop54-cpp)
