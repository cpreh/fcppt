...
inline Tuple init(Function const &_function)
{
  static_assert(fcppt::type_traits::is_std_tuple<Tuple>::value, "Tuple must be a tuple");

  return fcppt::container::tuple::detail::init<Tuple>(
      std::make_index_sequence<std::tuple_size<Tuple>::value>{}, _function);
}
