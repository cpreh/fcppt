...
{
  static_assert(
      fcppt::container::tree::is_object<std::remove_cv_t<Tree>>::value,
...
public:
  ...
  */
  explicit pre_order(Tree &_tree) : tree_{_tree} {}

  class iterator;

private:
  using tree_iterator = std::conditional_t<
      std::is_const_v<Tree>,
      typename Tree::const_reverse_iterator,
      typename Tree::reverse_iterator>;

  using tree_ref = fcppt::reference<Tree>;

  using optional_tree_ref = fcppt::optional::reference<Tree>;

  using stack_type = std::stack<tree_ref>;

  using iterator_base = fcppt::iterator::base<fcppt::iterator::types<
      iterator,
      fcppt::type_traits::value_type<tree_iterator>,
      typename tree_iterator::reference,
      typename tree_iterator::difference_type,
...
public:
  ...
  {
  public:
    iterator() : current_{}, positions_{} {}

    explicit iterator(optional_tree_ref const _current) : current_(_current), positions_() {}
    ...
    void increment()
    {
      reference cur_deref(this->dereference());
      ...
      if (!cur_deref.empty())
      {
        ...
        this->current_ = cur_deref.front();
      }
      else if (this->positions_.empty())
      {
        this->current_ = optional_tree_ref();
      }
      else
      {
        this->current_ = optional_tree_ref(this->positions_.top());

        this->positions_.pop();
      }
    }

    [[nodiscard]] reference dereference() const { return this->current_.get_unsafe().get(); }

    [[nodiscard]] bool equal(iterator const &_other) const
    {
      return this->current_ == _other.current_;
    }
  ...
  */
  [[nodiscard]] iterator begin() const { return iterator(optional_tree_ref(this->tree_)); }

  /**
  \brief Return a dummy iterator to stop the traversal
  */
  [[nodiscard]] iterator end() const { return iterator(optional_tree_ref()); }
