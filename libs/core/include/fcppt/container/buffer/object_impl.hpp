...
template <typename T, typename A>
fcppt::container::buffer::object<T, A>::object(size_type const _size, A _alloc)
    : impl_{_alloc, _alloc.allocate(_size), _size}
{
}

template <typename T, typename A>
fcppt::container::buffer::object<T, A>::object(object &&_other) noexcept
...
fcppt::container::buffer::object<T, A>::operator=(object &&_other) noexcept
{
  _other.swap(*this);
...
template <typename T, typename A>
void fcppt::container::buffer::object<T, A>::resize_write_area(size_type const _sz)
{
  ...
  size_type const new_size{std::max(
      fcppt::cast::size<size_type>(
  ...
  this->impl_ = std::move(new_impl);
}

template <typename T, typename A>
void fcppt::container::buffer::object<T, A>::swap(object &_other) noexcept
{
...
template <typename T, typename A>
void fcppt::container::buffer::object<T, A>::release_internal() noexcept
{
  this->impl_.first_ = nullptr;

  this->impl_.read_end_ = nullptr;

  this->impl_.write_end_ = nullptr;

  this->impl_.cap_ = nullptr;
}
...
template <typename T, typename A>
fcppt::container::buffer::object<T, A>::impl::impl(
    A const &_alloc, pointer const _first, size_type const _size)
    : alloc_{_alloc},
      first_{_first},
      read_end_{first_},
...
template <typename T, typename A>
fcppt::container::buffer::object<T, A>::impl::~impl() noexcept = default;

template <typename T, typename A>
void fcppt::container::buffer::object<T, A>::impl::deallocate() noexcept
{
  if (this->first_ != nullptr)
  {
    this->alloc_.deallocate(this->first_, fcppt::cast::to_unsigned(this->cap_ - this->first_));
  }
}

template <typename T, typename A>
void fcppt::container::buffer::swap(
    fcppt::container::buffer::object<T, A> &_x, fcppt::container::buffer::object<T, A> &_y) noexcept
{
  _x.swap(_y);
}
