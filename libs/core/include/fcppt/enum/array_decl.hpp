...
public:
  ...
  using internal = std::array<Value, static_size::value>;
  ...
  [[nodiscard]] reference operator[](Enum);

  [[nodiscard]] const_reference operator[](Enum) const;
  ...
  [[nodiscard]] internal const &impl() const;

  // Not private so array is still a POD
  [[nodiscard]] static typename internal::size_type to_index(Enum);
...
template <typename Enum, typename Value>
void swap(fcppt::enum_::array<Enum, Value> &, fcppt::enum_::array<Enum, Value> &);
