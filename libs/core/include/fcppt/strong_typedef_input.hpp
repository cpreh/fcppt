...
operator>>(std::basic_istream<Ch, Traits> &_stream, fcppt::strong_typedef<T, Tag> &_value)
{
  fcppt::optional::maybe_void(fcppt::io::extract<T>(_stream), [&_value](T &&_result) {
    _value = fcppt::strong_typedef<T, Tag>(std::move(_result));
  });
