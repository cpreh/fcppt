    ...
    fcppt::options::state &&_state, fcppt::options::parse_context const &_context) const
{
  return fcppt::either::map(
      fcppt::options::deref(parser_).parse(std::move(_state), _context),
...
template <typename Result, typename Parser>
fcppt::string fcppt::options::detail::concrete<Result, Parser>::usage() const
{
  return fcppt::options::deref(parser_).usage();
}
